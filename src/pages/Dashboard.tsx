    ...
    <AppLayout>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <button className="text-sm font-medium text-primary hover:underline">Manage Agents →</button>
        </div>
        <div className="grid grid-cols-5 gap-4">
          {agents.map((agent) => (
            <div key={agent.name} className="bg-card border border-border rounded-xl p-4">
      ...
      <div className="grid grid-cols-3 gap-5">
        ...
        <div className="col-span-2 bg-card border border-border rounded-xl p-5">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex gap-2">
              <button className="px-3 py-1.5 text-xs font-medium text-foreground bg-secondary rounded-lg">Filter</button>
              <button className="px-3 py-1.5 text-xs font-medium text-foreground bg-secondary rounded-lg">Sort</button>
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                {pipelineResearching.map((item) => (
                  <div key={item.title} className="bg-background border border-border rounded-lg p-3">
                    <p className="text-sm font-medium text-foreground mb-2">{item.title}</p>
                    <div className="flex items-center justify-between">
                      <div className="w-6 h-6 rounded-full bg-destructive/10 text-destructive text-xs flex items-center justify-center font-semibold">
                        {item.avatar}
                      </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                {pipelineDrafting.map((item) => (
                  <div key={item.title} className="bg-background border-2 border-primary/30 rounded-lg p-3">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="w-6 h-6 rounded-full bg-warning/10 text-warning text-xs flex items-center justify-center font-semibold">
                        {item.avatar}
                      </div>
                      <span className="text-xs text-muted-foreground">{item.time}</span>
                    </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5">
            ...
            {trending.map((item) => (
              <div key={item.title} className="flex items-start justify-between gap-3">
                ...
                <div className="text-right shrink-0">
                  <p className={`text-sm font-bold ${item.virality >= 90 ? "text-success" : item.virality >= 75 ? "text-primary" : "text-warning"}`}>
                    {item.virality}%
                  </p>
          ...
          <button className="w-full mt-5 py-2 text-sm font-medium text-foreground border border-border rounded-lg hover:bg-secondary transition-colors">
            View Full Report
          </button>
        </div>
      </div>
      ...
      {/* Estimated Reach */}
      <div className="mt-5 bg-primary rounded-xl p-6 text-primary-foreground">
